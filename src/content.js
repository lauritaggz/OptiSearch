  ...
  function main() {
    ...
    DOMAdapter.waitForResultsWithObserver((resultadosCrudos) => {
      try {
        ...
        // Paso 4b: SPIKE P01 — Telemetría de Proof of Access
        const p01Targets = resultadosCrudos.slice(0, 5);
        if (p01Targets.length > 0 && typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
          Promise.all(
            p01Targets.map((item) =>
              new Promise((resolve) => {
                chrome.runtime.sendMessage(
                  { type: 'OPTISEARCH_SPIKE_FETCH_PAGE', url: item.enlace },
                  (response) => {
                    if (chrome.runtime.lastError) {
                      ...
                      resolve(response || {
                        ...
                        elapsedMs: 0,
                        hasHtmlContent: false,
                        error: 'Sin respuesta del service worker'
                      });
                    }
                  }
                );
            ...
            const tableData = results.map((res, index) => {
              ...
              return {
                ...
                'Dominio': domain,
                'URL': res.url,
                'Resultado': res.ok ? 'OK' : 'FAILED',
                'Status': res.status ?? 'N/A',
                'Content-Type': res.contentType ? res.contentType.split(';')[0] : 'N/A',
                'Has HTML': res.hasHtmlContent,
                'Size (Bytes)': res.sizeBytes,
                'Tiempo (ms)': res.elapsedMs,
                'Error': res.error || 'None'
              };
            });
            console.log('[OptiSearch SPIKE][P01] Telemetría de Acceso a Contenido Externo:');
